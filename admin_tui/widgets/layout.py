"""Changelist column layout + cell truncation (v2 / FR-001..005, SC-001).

The v1 changelist let Textual's ``DataTable`` auto-size columns to their widest
cell and auto-scroll horizontally to the cursor — which read as "columns
cropped, full text revealed on select." v2 computes **fixed** column widths once
per rebuild (independent of which row is selected) and pre-truncates every cell.

Both functions are pure and unit-tested in ``tests/unit/test_layout.py``. The
critical invariant (SC-001): ``compute_column_widths`` output depends only on
...
def truncate_cell(value: object, width: int) -> str:
    ...
    depend on selection state (FR-002).
    """
    ...
    *,
    ...
    Args:
        ...
        rows: the *current page's* rendered rows as ``{column_key: value}``
            ...
            queryset — so memory stays bounded by ``list_per_page`` (SC-002).

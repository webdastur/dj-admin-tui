...
``ModelMultipleChoiceField`` validation ("Enter a list of values") — the core
"some model edits not working" defect (v2 / FR-012/013). A ``SelectionList``
returns a list of selected pks, which the save path now gathers correctly.
"""
...
def many_to_many_widget(bound_field: "BoundField") -> SelectionList[str]:
    ...
    for i, (val, label) in enumerate(field.choices):
        ...
        selections.append(
            Selection(str(label), str(pk), str(pk) in current_set)
        )

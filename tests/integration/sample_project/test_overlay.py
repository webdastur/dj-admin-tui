"""End-to-end overlay coverage (FR-020–024, SC-005, Constitution VIII).

Every FR-024 extension class has at least one assertion here. If any of
these break, the sample app fails — that's how Constitution VIII bites.
...
Plus:
  ...
  - zero-config still works when overlays are unregistered (FR-021).
  - Public API surface remains exactly 5 names (FR-030 / SC-006).
"""
...
def test_book_overlay_declares_row_action(with_library_overlays):
    """FR-024 slot 1: row_actions = ["mark_featured_via_tui"]."""
...
def test_book_overlay_declares_key_binding(with_library_overlays):
    """FR-024 slot 3: key_bindings includes ('f', 'mark_featured_via_tui', 'Feature')."""
    overlay = tui_site.get_or_synthesize(Book)
...
def test_book_overlay_declares_bulk_action(with_library_overlays):
    """FR-024 slot 2: bulk_actions = ["bulk_recolor"]."""
    overlay = tui_site.get_or_synthesize(Book)
...
@pytest.mark.django_db
def test_render_cell_override_bolds_featured_title(
...
def test_author_overlay_returns_custom_detail_screen(with_library_overlays):
    """FR-024 slot 6: AuthorTui.get_detail_screen → AuthorStatsScreen."""
...
@pytest.mark.django_db
def test_before_save_populates_normalised_title(
    with_library_overlays, db, author
):
    """FR-024 lifecycle hooks (7): before_save populates a field outside the form."""
    overlay = tui_site.get_or_synthesize(Book)
    request = build_request(_admin_user(db))
...
# ---- zero-config still works (FR-021) -----------------------
...
def test_unregistering_overlays_falls_back_to_default(with_library_overlays):
    ...
    the synthesized default (Constitution III)."""
...
# ---- public API surface unchanged (FR-030 / SC-006) ---------


@pytest.mark.django_db
def test_public_api_surface_unchanged_after_overlay_load(with_library_overlays):
    """Loading the sample overlays MUST NOT add anything to admin_tui.__all__."""
    import admin_tui

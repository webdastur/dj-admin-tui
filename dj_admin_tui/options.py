...
from dj_admin_tui.sites import AlreadyRegistered, tui_site
...
if TYPE_CHECKING:
    ...
    from dj_admin_tui.sites import TuiSite
...
class TuiAdmin:
    ...
    default delegates to it (no parallel logic).
    """

    # -- declarative slots (each defaults to None / [] / {}) ------------

    list_columns: ClassVar[Sequence[str] | None] = None
    detail_fieldsets: ClassVar[Any | None] = None
    row_actions: ClassVar[list[str]] = []
    bulk_actions: ClassVar[list[str]] = []
    key_bindings: ClassVar[list[tuple[str, str, str]]] = []
    field_widgets: ClassVar[dict[Any, Callable[..., Widget]]] = {}
    ...
    def get_changelist_screen(
        ...
        Default returns `dj_admin_tui.screens.changelist.ChangelistScreen`.
    ...
    *models: type[Model],
    ...
    def decorator(overlay_cls: type[TuiAdmin]) -> type[TuiAdmin]:
        if not issubclass(overlay_cls, TuiAdmin):
            raise TypeError(f"{overlay_cls.__name__} must be a subclass of TuiAdmin.")
        for model in models:
...
__all__ = ["AlreadyRegistered", "BoundCellValue", "TuiAdmin", "register"]

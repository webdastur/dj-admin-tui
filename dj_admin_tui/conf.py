...
_DEFAULTS: dict[str, Any] = {
    "APP_CLASS": "dj_admin_tui.app.AdminTuiApp",
    "PAGE_SIZE": 50,
    "THEME_NAME": None,
    "THEME": None,
    "AUTODISCOVER": True,
...
def _load() -> dict[str, Any]:
    ...
    if not isinstance(raw, dict):
        raise ImproperlyConfigured(f"ADMIN_TUI must be a dict, got {type(raw).__name__}.")

    unknown = set(raw) - _VALID_KEYS
    if unknown:
        raise ImproperlyConfigured(
            f"Unknown ADMIN_TUI keys: {sorted(unknown)}. Valid keys: {sorted(_VALID_KEYS)}."
        )
    ...
    if not isinstance(page_size, int) or not (1 <= page_size <= 10_000):
        raise ImproperlyConfigured(
            f"ADMIN_TUI['PAGE_SIZE'] must be an int in [1, 10000], got {page_size!r}."
        )
    ...
    if theme_name is not None:
        if not isinstance(theme_name, str):
            raise ImproperlyConfigured(
                f"ADMIN_TUI['THEME_NAME'] must be a str or None, got {theme_name!r}."
            )
        from dj_admin_tui.themes import valid_theme_names
    ...
    if theme is not None:
        ...
        if not theme_path.is_file():
            raise ImproperlyConfigured(f"ADMIN_TUI['THEME'] does not exist: {theme_path}")
        if theme_path.suffix != ".tcss":
            raise ImproperlyConfigured(f"ADMIN_TUI['THEME'] must be a .tcss file, got {theme_path}")
        merged["THEME"] = theme_path

    if not isinstance(merged["AUTODISCOVER"], bool):
        raise ImproperlyConfigured(
            f"ADMIN_TUI['AUTODISCOVER'] must be bool, got {merged['AUTODISCOVER']!r}."
        )

    if not isinstance(merged["COMPAT_WARNINGS"], bool):
        raise ImproperlyConfigured(
            f"ADMIN_TUI['COMPAT_WARNINGS'] must be bool, got {merged['COMPAT_WARNINGS']!r}."
        )

    # APP_CLASS subclass check happens lazily in `resolve_app_class()` to
    # avoid an import cycle with dj_admin_tui.app at module-load time.
...
def resolve_app_class() -> type:
    ...
    Performed lazily to break the dj_admin_tui.conf → dj_admin_tui.app import
    cycle at AppConfig.ready() time.
    """
    from dj_admin_tui.app import AdminTuiApp  # local to break cycle
    ...
    if not isinstance(cls, type) or not issubclass(cls, AdminTuiApp):
        raise ImproperlyConfigured(
            ...
            f"subclass of dj_admin_tui.AdminTuiApp."
        )
    return cls

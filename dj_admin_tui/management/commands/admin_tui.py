...
exit BEFORE Textual is constructed.
"""
    ...
    def handle(self, *args: Any, **options: Any) -> None:
        ...
        if theme_arg is not None:
            ...
            if not theme_path.is_file() or theme_path.suffix != ".tcss":
                _stderr(f"--theme path {theme_path} is not a readable .tcss file.")
                sys.exit(5)
        ...
        if theme_name is not None:
            ...
            if theme_name not in valid_theme_names():
                _stderr(f"--theme-name {theme_name!r} is not a registered theme.")
                sys.exit(5)
...
def _resolve_user(requested: str | None) -> Any:
    ...
    candidates = list(User.objects.filter(is_superuser=True, is_active=True, is_staff=True))
    if len(candidates) == 1:
        return candidates[0]
    if not candidates:
        _stderr(

...
defaults travel the same code path as third-party overlays.
...
class TuiSite:
    ...
    The default singleton is `dj_admin_tui.tui_site`. Custom sites are not
    part of the public API at v1.0.
    """
    ...
    def register(
        ...
        if model in self._registry:
            raise AlreadyRegistered(
                f"{model.__module__}.{model.__name__} is already registered on this TuiSite."
            )
    ...
    def register_screen(self, slug: str, screen_cls: type[Screen]) -> None:
        ...
        if slug in self._screens:
            raise AlreadyRegistered(f"Global tool screen slug {slug!r} is already registered.")
    ...
    def get_or_synthesize(self, model: type[Model]) -> TuiAdmin:
        ...
        slots delegate to the underlying `ModelAdmin` (zero-config). Cached
        so successive lookups return the same instance.
        """
        if model in self._registry:
...
#: The default process-wide TuiSite. Public API.
tui_site = TuiSite()

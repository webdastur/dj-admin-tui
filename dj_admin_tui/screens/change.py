...
Three modes:
  - `view`  : read-only fieldsets as `name: value` text.
...
On a successful save we call `save_model`,
...
from dj_admin_tui.screens.action_confirm import DELETE_SENTINEL, ActionConfirmScreen
from dj_admin_tui.widgets.layout import sanitize_cell
...
class ChangeScreen(Screen):
        ...
        with VerticalScroll(id="detail-body"):
            ...
            if self.mode in ("add", "edit"):
                with Horizontal(id="form-buttons"):
                    yield Button("Save", id="save-button", classes="atui-btn atui-btn-primary")
                    yield Button(
                    ...
                    yield Button("Cancel", id="cancel-button", classes="atui-btn atui-btn-default")
                    if self.mode == "edit" and self.overlay.has_delete_permission(
                        ...
                        yield Button(
                            "Delete", id="delete-button", classes="atui-btn atui-btn-danger"
                        )
        yield Footer()
    ...
    def _compose_body(self) -> Iterable[Widget]:
        if self.mode == "view":
            ...
            self.form = _build_form(self.overlay.model_admin, self.request, obj=self.obj)
            self._readonly = _readonly_field_names(self.overlay.model_admin, self.request, self.obj)
    ...
    def _compose_view_body(self) -> Iterable[Widget]:
        ...
        # Inline relations rendered as read-only sections below the
    ...
    def _compose_view_field(self, field_name: str) -> Iterable[Widget]:
        ...
        with Horizontal(classes="field-row"):
            yield Static(f"{escape(self._readonly_label(field_name))}:", classes="field-label")
            yield Static(escape(self._readonly_value(field_name)), classes="field-value")

    def _compose_inlines(self) -> Iterable[Widget]:
        inlines = _inline_instances(self.overlay.model_admin, self.request, self.obj)
            ...
            with Vertical(classes="fieldset inline-section"):
                ...
                if count == 0:
                    ...
                    if not fields:
                        ...
                        fields = [
                            f.name
                            for f in inline.model._meta.fields
    ...
    def _related_queryset(self, inline) -> Any:  # type: ignore[no-untyped-def]
        ...
        fk_name = inline.fk_name or _detect_fk_to(inline.model, type(self.obj))
        if fk_name is None:
            return inline.model._default_manager.none()
        return inline.model._default_manager.filter(**{fk_name: self.obj.pk})
        ...
        with Horizontal(classes="field-row"):
            ...
            with Vertical(classes="field-widget"):
                if name in self._readonly:
                    ...
                    yield Static(escape(self._readonly_value(name)), classes="readonly")
                else:
    ...
    def _do_save(self) -> Any | None:
        ...
        if self.mode == "add" and not self.overlay.has_add_permission(self.request):
            return None
        if self.mode == "edit" and not self.overlay.has_change_permission(self.request, self.obj):
            return None

        data = self._gather_data()
        # Re-bind the form with the gathered data so `is_valid()` runs.
        bound_form = _build_form(self.overlay.model_admin, self.request, obj=self.obj, data=data)
        ...
        try:
            ...
            self.overlay.model_admin.save_related(self.request, bound_form, [], change=not is_add)

            change_message = _change_message(
                self.overlay.model_admin, self.request, bound_form, [], add=is_add
            )
            if is_add:
                _log_addition(self.overlay.model_admin, self.request, new_obj, change_message)
            else:
                _log_change(self.overlay.model_admin, self.request, new_obj, change_message)
    ...
    def action_edit(self) -> None:
        ...
        if not self.overlay.has_change_permission(self.request, self.obj):
            return
        self.app.pop_screen()
        screen_cls = self.app.screen_for_detail(self.overlay, self.request, self.obj)
        self.app.push_screen(
            screen_cls(
                session=self.session,
                overlay=self.overlay,

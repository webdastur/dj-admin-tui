...
truth — we never recompute filtering or ordering.

Presentation + interaction layer over that behavior:
  - Columns get **fixed, selection-independent widths** and every cell is
    **pre-truncated** (`dj_admin_tui.widgets.layout`), avoiding the glitch where
    DataTable auto-sized columns and auto-scrolled to the cursor cell.
  - A fixed footer bar previews the focused row's full (untruncated) values with
    no reflow.
  - A Django-style filter sidebar (`dj_admin_tui.widgets.filters`) renders the admin's
    own `list_filter` choices.
  - Object-tools (Add) + pagination controls are clickable Buttons.
  - Mouse: single click focuses a row, clicking the focused row / Enter opens it,
    a click on the checkbox column toggles selection, a header click sorts.
"""
...
from dj_admin_tui.core.actions import _get_actions
...
class _SearchModal(ModalScreen[str]):
    ...
    BINDINGS: ClassVar[list[BindingType]] = [Binding("escape", "dismiss", "Cancel", show=True)]
...
class ChangelistScreen(Screen):
    ...
    def on_mouse_down(self, event) -> None:  # type: ignore[no-untyped-def]
        ...
        checkbox can be told apart from a keyboard Enter."""
    ...
    def on_click(self, event) -> None:  # type: ignore[no-untyped-def]
        ...
        click is handled by RowSelected → open."""
        meta = getattr(event.style, "meta", {}) or {}
        self._down_column = None
    ...
    def on_data_table_row_highlighted(self, event: DataTable.RowHighlighted) -> None:
        """Update the footer cell-preview when the cursor moves."""
        pk = event.row_key.value if event.row_key is not None else None
    ...
    def on_filter_sidebar_filter_chosen(self, event: FilterSidebar.FilterChosen) -> None:
        """Apply Django's own filter query string."""
    ...
    def on_key(self, event) -> None:  # type: ignore[no-untyped-def]
        """Dispatch overlay-declared `key_bindings`.
    ...
    def _refresh_header(self, changelist) -> None:  # type: ignore[no-untyped-def]
        ...
        self.query_one("#page-indicator", Static).update(f"page {page} / {num_pages}")
    ...
    def _refresh_table(self, changelist) -> None:  # type: ignore[no-untyped-def]
        ...
        for obj in objs:
            ...
            for col in columns:
                disp = sanitize_cell(self.overlay.render_cell(self.request, obj, col).display)
                cells[col] = disp
    ...
    def _sort_by_column_index(self, col_index: int) -> None:
        ...
        column). Reuses Django's `o` query param."""
    ...
    def _refresh_selection_indicator(
        ...
        marker = "✓" if pk in self.selected_pks else " "
        # The cell may not exist if a rebuild happened between the key press
        # and this call; swallowing that is safe.
        with contextlib.suppress(Exception):
    ...
    def _available_actions(self) -> list[tuple[str, str, str]]:
        ...
        # TUI-native bulk actions declared on the overlay.
    ...
    def _open_confirm(self, kind: str, action_name: str, action_label: str) -> None:
    ...
    def on_screen_resume(self) -> None:
        ...
        try:
            if self.selected_pks:
                ...
                existing = set(
                    str(pk)
                    for pk in qs.filter(pk__in=self.selected_pks).values_list("pk", flat=True)
                )

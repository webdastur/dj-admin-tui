...
from dj_admin_tui.core.actions import _run_action, _run_tui_action
from dj_admin_tui.core.audit import _log_deletions
...
class ActionConfirmScreen(Screen):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Vertical(id="confirm-body"):
            ...
            with Horizontal(id="confirm-buttons"):
                ...
                yield Button(no_label, id="no-button", classes="atui-btn atui-btn-default")
        yield Footer()
    ...
    def _do_action(self) -> None:
        if self.kind == "tui":
            result = _run_tui_action(self.overlay, self.request, self.action_name, self.queryset)
        else:
            result = _run_action(self.overlay, self.request, self.action_name, self.queryset)
        ...
        if result.exception is not None:
            # Recoverable state: stay on this screen, show the
            # error inline, restore the "Back" affordance.
            self._show_error(f"{type(result.exception).__name__}: {result.exception}")
            return

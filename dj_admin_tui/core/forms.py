"""Build the admin's ModelForm — reuse Django's admin.
...
def _inline_instances(
    ...
    permission gate the web admin's change_view applies.
    """
    instances = model_admin.get_inline_instances(request, obj)
    return [inline for inline in instances if inline.has_view_permission(request, obj)]

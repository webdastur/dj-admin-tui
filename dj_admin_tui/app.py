"""AdminTuiApp — the Textual App subclass. Public API.
...
Module-level imports deliberately avoid `dj_admin_tui.screens.*` — those are
loaded lazily in `on_mount()` so the package stays importable without them.
"""
...
#: component styling. See dj_admin_tui/styles.tcss.
...
def _allow_django_orm_in_async_context() -> None:
    ...
    ORM, which is what every admin call expects.
    """
...
class AdminTuiApp(App):
    ...
    def __init__(self, *, session: TuiSession) -> None:
        ...
        # no new public dj_admin_tui name.
        from dj_admin_tui.themes import register_bundled_themes, resolve_theme_name
    ...
    def on_mount(self) -> None:
        # Lazy import — keeps the package importable without the screens module.
    ...
    def action_show_help(self) -> None:
        self.notify(
            ...
            "See the documentation for the full keymap.",
            title="dj-admin-tui",
        )

    # --- screen-routing helpers (consulted by IndexScreen / Changelist).
    #
    # These exist so that defaults travel the extension path: overlays'
    # `get_changelist_screen` / `get_detail_screen` always decide which
    # Screen class is instantiated.
